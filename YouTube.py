...
def get_channel_details(c_id):
    request = youtube.channels().list(part="snippet,status,contentDetails,statistics", 
                                      id=c_id)
...
menu = ['Data Collection and Storage', 'Query Data']
choice = st.sidebar.radio("Select an option", menu)

if choice == 'Data Collection and Storage':
    
    st.title(':white[YouTube] :red[_Data Harvesting & Warehousing_]')
    ...
    if selected_query:
        if selected_query.startswith("1."):
            ...
            query = """
            SELECT v.video_name, IFNULL(COUNT(co.comment_id), 0) AS num_comments 
            FROM video v
            LEFT JOIN comment co ON v.video_id = co.video_id 
